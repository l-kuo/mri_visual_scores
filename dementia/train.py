...
def get_transforms():
    ...
    return Compose(base), Compose(base)
...
def train(rank, config):
    ...
    train_df = pd.read_csv(config.train_csv, usecols=['filepath', 'label'])
    val_df = pd.read_csv(config.val_csv, usecols=['filepath', 'label'])
    ...
    if rank == 0:
        print(f"Model: {config.model_name} | Train: {len(train_df)} | Val: {len(val_df)}")
    ...
    if rank == 0:
        logger = Logger(config.model_name, config.version_name)
        print_gpu_status()
    ...
    for epoch in range(config.epochs):
        ...
        if rank == 0:
            ...
            logger.log(epoch, 'Training', train_loss)
        ...
        if rank == 0:
            ...
            logger.log(epoch, 'Validation', val_loss)
            logger.log_lr(epoch, current_lr)
            logger.display_status(epoch, 'Validation', val_loss)
