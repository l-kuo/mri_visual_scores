...
def train(rank, config):
    ...
    scores = [('mta', config.mta_classes), ('erica', config.erica_classes)]

    train_df = pd.read_csv(config.mta_erica_train_csv, usecols=['filepath', 'MTA', 'ERICA'])
    val_df = pd.read_csv(config.mta_erica_val_csv, usecols=['filepath', 'MTA', 'ERICA'])
    ...
    if rank == 0:
        print(f"MTA+ERICA | Train: {len(train_df)} | Val: {len(val_df)}")

    train_transforms, val_transforms = get_transforms()

    train_ds = PersistentDataset(
        data=train_df.to_dict('records'), transform=train_transforms,
        cache_dir=os.path.join(config.mta_erica_cache_dir, 'train')
    )
    val_ds = PersistentDataset(
        data=val_df.to_dict('records'), transform=val_transforms,
        cache_dir=os.path.join(config.mta_erica_cache_dir, 'val')
    )
    ...
    if rank == 0:
        logger = Logger('mta_erica', config.mta_erica_version)
    ...
    for epoch in range(config.epochs):
        ...
        score_corrects = {name: 0.0 for name, _ in scores}
        ...
        for data in train_loop:
            ...
            loss_sum = sum(
                loss_fn(output[name], data[name.upper()].long().to(rank))
            ...
            for name, _ in scores:
                preds = torch.argmax(output[name], dim=1).detach().cpu()
                score_corrects[name] += (data[name.upper()] == preds).float().mean().item()

            running_train_loss += loss_sum.item()
            train_loop.set_postfix(loss=loss_sum.item())

        if rank == 0:
            train_loss = running_train_loss / len(train_loader)
            logger.log(epoch, 'Train Loss', train_loss)
            logger.display_status(epoch, 'Train Loss', train_loss)
            for name, _ in scores:
                acc = score_corrects[name] / len(train_loader) * 100
                print(f"  Train {name.upper()} Acc: {acc:.2f}%")

        model.eval()
        running_val_loss = 0.0
        score_gts = {name: [] for name, _ in scores}
        score_preds = {name: [] for name, _ in scores}
        ...
        with torch.no_grad():
            for data in val_loop:
                ...
                if rank == 0:
                    for name, _ in scores:
                        score_gts[name].extend(data[name.upper()].numpy())
                        score_preds[name].extend(torch.argmax(output[name], dim=1).cpu().numpy())

                val_loop.set_postfix(loss=loss_sum.item())
        ...
        if rank == 0:
            ...
            for name, _ in scores:
                ...
                print(classification_report(score_gts[name], score_preds[name], digits=4))
                acc = accuracy_score(score_gts[name], score_preds[name])
                f1 = f1_score(score_gts[name], score_preds[name], average='macro')
                acc_list.append(acc)
                logger.log(epoch, f'Val {name.upper()} Acc', acc)
                logger.log(epoch, f'Val {name.upper()} F1', f1)
            ...
            if mean_acc > best_acc:
                ...
                print(f"Saved best model at epoch {epoch} (mean acc={best_acc:.4f}).")
            else:
                patient += 1
    ...
    if rank == 0:
        ...
        print(f"Best val mean acc at epoch {best_epoch}: {best_acc:.4f}")
        logger.close()
